 ...
 */
const validateEnvironmentVariables = (): StripeConfig => {
...
const stripeClient = new Stripe(config.secretKey, {
  apiVersion: (config.apiVersion || DEFAULT_API_VERSION) as Stripe.LatestApiVersion,
  typescript: true,
  maxNetworkRetries: 2,
  timeout: 30000,
});
  ...
  if (error instanceof Stripe.errors.StripeError) {
    ...
    if (error instanceof Stripe.errors.StripeRateLimitError) {
      throw new BadRequestError(
        'Demasiadas solicitudes a Stripe. Por favor, intenta nuevamente en unos momentos.'
      );
    }
    ...
    if (error instanceof Stripe.errors.StripeAPIError) {
      throw new BadRequestError(
        'Error de conexión con el servicio de pagos. Por favor, intenta nuevamente más tarde.'
      );
    }
    ...
    if (error instanceof Stripe.errors.StripeConnectionError) {
      throw new BadRequestError(
        'Error de conexión con Stripe. Verifica tu conexión a internet e intenta nuevamente.'
      );
    }
 ...
 */
export const testStripeConnection = async (): Promise<void> => {
  try {
...
export const stripeConfig: Readonly<StripeConfig> = config;
