...
} from '@/modules/subscriptions/validators/subscription-plan.validator';
 ...
 */
export const assertPlanNotInUse = async (planId: UUID): Promise<void> => {
  const count = await Subscription.count({
    where: {
      planId,
      status: { [Op.in]: ['active', 'trialing'] as const },
    },
  });

  if (count > 0) {
    throw new ConflictError(
      'El plan tiene suscripciones activas y no puede desactivarse ni eliminarse',
      { planId, activeSubscriptionsCount: count }
    );
  }
};
...
export const listPlans = async (
  filters: ListSubscriptionPlansDTO
): Promise<{ data: SubscriptionPlan[]; pagination: PaginationMeta }> => {
  ...
  const totalPages = Math.ceil((result.count as number) / limit);
 ...
 */
export const assertPlanExistsAndActive = async (planId: UUID): Promise<SubscriptionPlan> => {
  const plan = await getPlanById(planId);
  if (!plan.active) {
    throw new ValidationError(
      'El plan no está activo y no puede utilizarse para suscripciones',
      'planId',
 ...
 * @returns Plan creado
 */
export const createPlan = async (data: CreateSubscriptionPlanDTO): Promise<SubscriptionPlan> => {
 ...
 * @param planId - ID del plan
 */
export const deletePlan = async (planId: UUID): Promise<void> => {
  const plan = await getPlanById(planId);
  await assertPlanNotInUse(planId);
  await plan.destroy();
 ...
 */
export const syncPlanToStripe = async (planId: UUID): Promise<SubscriptionPlan> => {
  const plan = await getPlanById(planId);
  ...
  try {
    if (!plan.stripeProductId) {
      ...
      if (hasPrices) {
        ...
        logger.info(
          { planId: plan.id, stripeProductId: product.id },
          'Plan sincronizado con Stripe (solo Product; sin precios, ej. Enterprise)'
        );
      }
    } else {
      ...
      logger.info(
        { planId: plan.id, stripeProductId: plan.stripeProductId },
        'Plan sincronizado con Stripe (Product actualizado)'
      );
    }
  } catch (error) {
