 ...
 * Requiere autenticación y acceso a la organización.
 */
export const getCurrentSubscription = async (req: Request, res: Response): Promise<Response> => {
  if (!req.user) {
    return res.status(401).json({
      success: false,
      error: 'No autorizado',
  ...
  const subscription = await subscriptionService.getSubscriptionByOrganization(
    organizationId,
    userId
  );
...
export const cancelSubscription = async (req: Request, res: Response): Promise<Response> => {
  ...
  const subscriptionId = req.params['subscriptionId'] as string;
  const userId = req.user.userId;
  const data = req.body as CancelSubscriptionDTO;
 ...
 * Requiere autenticación. Valida que el usuario tenga acceso a la organización de la suscripción.
 */
export const reactivateSubscription = async (req: Request, res: Response): Promise<Response> => {
  if (!req.user) {
    return res.status(401).json({
      success: false,
      error: 'No autorizado',
