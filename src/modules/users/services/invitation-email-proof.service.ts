...
import {
  ...
  INVITATION_OTP_EXPIRES_MINUTES,
 ...
 */
const generateOtp = async (): Promise<{ otp: string; otpHash: string }> => {
 ...
 */
const generateProofToken = async (): Promise<{ proof: string; proofHash: string }> => {
 ...
 */
const assertInvitationValidForEmailProof = async (
  invitationId: UUID,
  email: string
): Promise<Invitation> => {
...
export const startVerifyEmail = async (
  ...
  if (
    ...
    throw new ValidationError(
      'Ya se envió un código recientemente. Espera unos minutos o usa el código que recibiste.',
      undefined,
      { retryAfterMinutes: Math.ceil((existing.otpExpiresAt.getTime() - Date.now()) / 60000) }
    );
  }
  ...
  logger.info(
    ...
    'Verificación de email por invitación iniciada, OTP enviado'
  );
 ...
 */
export const confirmVerifyEmail = async (
  invitationId: UUID,
  email: string,
  otp: string
): Promise<ConfirmVerifyEmailResult> => {
  const emailNormalized = email.trim().toLowerCase();
  ...
  logger.info(
    { invitationId, email: emailNormalized },
    'Proof de invitación emitido tras OTP correcto'
  );
 ...
 */
export const consumeProofForRegistration = async (
  invitationId: UUID,
  email: string,
  invitationProof: string
