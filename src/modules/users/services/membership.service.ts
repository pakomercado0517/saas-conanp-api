import type { UUID, Role } from '@/shared/database/types.js';
...
import type {
  ...
  UpdateMembershipDTO,
  ListMembershipsDTO,
} from '@/modules/users/validators/membership.validator.js';
import {
  ForbiddenError,
  NotFoundError,
  ConflictError,
 ...
 */
export const assertIsAdmin = async (userId: UUID, areaId: UUID): Promise<void> => {
 ...
 */
export const isUserAdminInArea = async (userId: UUID, areaId: UUID): Promise<boolean> => {
  const membership = await Membership.findOne({
    where: {
      userId,
      areaId,
...
export const updateMembershipRole = async (
  ...
  const updateData: Partial<{ role: Role; status: 'activo' | 'inactivo' | 'suspendido' }> = {};
...
export const listMemberships = async (
  ...
  userId: UUID
): Promise<{ data: Membership[]; pagination: PaginationMeta }> => {
