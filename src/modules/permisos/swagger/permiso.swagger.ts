...
const PermisoSchema = registry.register(
  'Permiso',
  z.object({
...
const PermisoBulkCreatePayloadSchema = registry.register(
  'PermisoBulkCreatePayload',
  z.object({
    created: z
      .array(PermisoSchema)
...
const PermisoCreateResponseSchema = registry.register(
...
const PermisoResponseSchema = registry.register(
  'PermisoResponse',
  z.object({
    success: z.literal(true),
    data: PermisoSchema,
...
const PermisoListResponseSchema = registry.register(
  'PermisoListResponse',
  z.object({
    success: z.literal(true),
...
registry.registerPath({
  ...
  description:
    'Crea un permiso que autoriza a un prestador para realizar una actividad durante un periodo. Con `appliesToAllAreas: true`, se crea una fila por cada área de la dependencia emparejando actividades por nombre normalizado; la respuesta incluye `created` y `permissionGroupId`.',
  security: [{ bearerAuth: [] }],
  request: {
  ...
  responses: {
    201: {
      description:
        'Permiso creado (una fila) o permisos materializados (varias filas con el mismo permissionGroupId)',
      content: {
        'application/json': {
