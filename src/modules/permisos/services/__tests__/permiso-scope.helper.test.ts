...
import {
  ...
  findMatchingActividadesByDependencia,
} from '../permiso-scope.helper.js';

const DEP_ID = '11111111-1111-1111-1111-111111111111' as UUID;
const AREA1 = 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa' as UUID;
const AREA2 = 'bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb' as UUID;
const REF_ACT = 'cccccccc-cccc-cccc-cccc-cccccccccccc' as UUID;
const ACT_A1 = 'dddddddd-dddd-dddd-dddd-dddddddddddd' as UUID;
const ACT_A2 = 'eeeeeeee-eeee-eeee-eeee-eeeeeeeeeeee' as UUID;
...
vi.mock('@/modules/actividades/models/actividad.model.js', () => ({
  Actividad: {
    findByPk: (...args: unknown[]): unknown => mockActividadFindByPk(...args),
    findAll: (...args: unknown[]): unknown => mockActividadFindAll(...args),
  },
}));
...
describe('permiso-scope.helper', () => {
  ...
  describe('findMatchingActividadesByDependencia', () => {
    ...
    it('lanza ValidationError si falta actividad con el mismo nombre en algún área', async () => {
      ...
      await expect(findMatchingActividadesByDependencia(DEP_ID, REF_ACT)).rejects.toThrow(
        ValidationError
      );
    });
  });
