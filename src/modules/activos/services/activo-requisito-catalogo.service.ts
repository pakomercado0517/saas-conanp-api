...
export const listCatalogo = async (
  organizationId: UUID,
  filters: ListActivoRequisitoCatalogoDTO,
  requestingUserId: UUID
): Promise<ActivoRequisitoCatalogo[]> => {
 ...
 */
export const createCatalogoEntry = async (
  organizationId: UUID,
  data: CreateActivoRequisitoCatalogoDTO,
  requestingUserId: UUID
): Promise<ActivoRequisitoCatalogo> => {
  await assertCanAccessOrganization(requestingUserId, organizationId);
  const dependenciaId = await getDependenciaIdFromAreaId(organizationId);
  ...
  if (existing) {
    throw new ValidationError(
      'Ya existe una entrada en el catálogo con esta clave para este tipo de activo',
      undefined,
      { dependenciaId, tipoActivo: data.tipoActivo, key: data.key }
    );
  }
 ...
 */
export const updateCatalogoEntry = async (
  organizationId: UUID,
  catalogoId: UUID,
  data: UpdateActivoRequisitoCatalogoDTO,
  requestingUserId: UUID
): Promise<ActivoRequisitoCatalogo> => {
  await assertCanAccessOrganization(requestingUserId, organizationId);
  const dependenciaId = await getDependenciaIdFromAreaId(organizationId);
  ...
  if (data.tipoDato !== undefined) updateData.tipoDato = data.tipoDato;
  if (data.requerido !== undefined) updateData.requerido = data.requerido;
  if (data.requiereDocumento !== undefined) updateData.requiereDocumento = data.requiereDocumento;
 ...
 */
export const deleteCatalogoEntry = async (
  organizationId: UUID,
  catalogoId: UUID,
  requestingUserId: UUID
): Promise<void> => {
  ...
  logger.info(
    { catalogoId, dependenciaId, organizationId, requestingUserId },
    'Entrada de catálogo de requisitos eliminada'
  );
};
 ...
 */
export const getCatalogoForActivo = async (
  dependenciaId: UUID,
