...
import type { ActivoStatus } from '../activo.service.js';
...
describe('activo.service', () => {
  ...
  describe('validateEstadoTransition', () => {
    const validTransitions: [ActivoStatus, ActivoStatus][] = [
      ['pendiente', 'aprobado'],
      ['pendiente', 'rechazado'],
      ['aprobado', 'suspendido'],
      ['rechazado', 'pendiente'],
    ...
    it('throws ValidationError for invalid transition pendiente -> suspendido', () => {
      expect(() => activoService.validateEstadoTransition('pendiente', 'suspendido')).toThrow(
        ValidationError
      );
    });

    it('throws ValidationError for invalid transition aprobado -> pendiente', () => {
      expect(() => activoService.validateEstadoTransition('aprobado', 'pendiente')).toThrow(
        ValidationError
      );
    });

    it('throws ValidationError for invalid transition rechazado -> aprobado', () => {
      expect(() => activoService.validateEstadoTransition('rechazado', 'aprobado')).toThrow(
        ValidationError
      );
    });
  });
  ...
  describe('validateActivoAprobado', () => {
    it('throws NotFoundError when activo does not exist', async () => {
      ...
      await expect(activoService.validateActivoAprobado(ACTIVO_ID, ORG_ID)).rejects.toThrow(
        NotFoundError
      );
    });
  ...
  describe('createActivo', () => {
    ...
    it('throws ValidationError when organizationId in data does not match parameter', async () => {
      const otherOrgId = 'ffffffff-ffff-ffff-ffff-ffffffffffff' as UUID;

      await expect(
        activoService.createActivo({ ...createData, organizationId: otherOrgId }, ORG_ID, USER_ID)
    ...
    it('throws NotFoundError when prestador does not exist', async () => {
      ...
      await expect(activoService.createActivo(createData, ORG_ID, USER_ID)).rejects.toThrow(
        NotFoundError
      );
    });
    ...
    it('creates activo and returns it with relations on success', async () => {
      ...
      (createdActivo.reload as ReturnType<typeof vi.fn>).mockResolvedValue(reloadedActivo);
  ...
  describe('getActivoById', () => {
    it('throws NotFoundError when activo does not exist', async () => {
      ...
      await expect(activoService.getActivoById(ACTIVO_ID, ORG_ID, USER_ID)).rejects.toThrow(
        NotFoundError
      );
    });
  ...
  describe('updateActivoStatus', () => {
    it('throws NotFoundError when activo does not exist', async () => {
      ...
      await expect(
        activoService.updateActivoStatus(ACTIVO_ID, ORG_ID, 'aprobado', USER_ID)
    ...
    it('throws ValidationError for invalid transition', async () => {
      ...
      await expect(
        activoService.updateActivoStatus(ACTIVO_ID, ORG_ID, 'suspendido', USER_ID)
      ).rejects.toThrow(ValidationError);
    });
    ...
    it('updates status and returns activo on valid transition', async () => {
      ...
      (activoRecord.reload as ReturnType<typeof vi.fn>).mockResolvedValue(reloadedActivo);
  ...
  describe('updateActivo', () => {
    it('throws ValidationError for invalid status transition', async () => {
      ...
      await expect(
        activoService.updateActivo(ACTIVO_ID, ORG_ID, { status: 'pendiente' }, USER_ID)
      ).rejects.toThrow(ValidationError);
    });
    ...
    it('updates activo and returns it on success', async () => {
      ...
      (activoRecord.reload as ReturnType<typeof vi.fn>).mockResolvedValue(updatedActivo);

      const result = await activoService.updateActivo(
        ACTIVO_ID,
        ORG_ID,
        { type: 'embarcacion' },
        USER_ID
      ...
      expect(activoRecord.update).toHaveBeenCalledWith(
        expect.objectContaining({ type: 'embarcacion' })
      );
      expect(result).toBeDefined();
    });
  });
  ...
  describe('deleteActivo', () => {
    it('throws NotFoundError when activo does not exist', async () => {
      ...
      await expect(activoService.deleteActivo(ACTIVO_ID, ORG_ID, USER_ID)).rejects.toThrow(
        NotFoundError
      );
    });
