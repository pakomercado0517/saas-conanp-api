...
export const createActivo = async (req: Request, res: Response): Promise<Response> => {
  ...
  const activoData: CreateActivoDTO = {
    ...data,
    organizationId,
  };
 ...
 * }
 */
export const listActivos = async (req: Request, res: Response): Promise<Response> => {
  if (!req.user) {
    return res.status(401).json({
      success: false,
      error: 'No autorizado',
  ...
  const filters =
    (req.validatedQuery as ListActivosDTO | undefined) ?? (req.query as unknown as ListActivosDTO);
 ...
 * }
 */
export const updateActivo = async (req: Request, res: Response): Promise<Response> => {
  if (!req.user) {
    return res.status(401).json({
      success: false,
      error: 'No autorizado',
  ...
  const activoId = req.params['activoId'] as string;
  const userId = req.user.userId;
  const data = req.body as UpdateActivoDTO;
 ...
 */
export const deleteActivo = async (req: Request, res: Response): Promise<Response> => {
  if (!req.user) {
    return res.status(401).json({
      success: false,
      error: 'No autorizado',
 ...
 * }
 */
export const approveActivo = async (req: Request, res: Response): Promise<Response> => {
  if (!req.user) {
    return res.status(401).json({
      success: false,
      error: 'No autorizado',
  ...
  const activo = await activoService.updateActivoStatus(
    activoId,
    organizationId,
    'aprobado',
    userId
