...
const PRESTADOR_ID = 'eeeeeeee-eeee-eeee-eeee-eeeeeeeeeeee' as UUID;
...
describe('evento.service', () => {
  ...
  describe('createEvento', () => {
    ...
    it('creates evento with BLOQUES agenda on success', async () => {
      ...
      (createdEvento.reload as ReturnType<typeof vi.fn>).mockResolvedValue(createdEvento);
      ...
      expect(mockEventoOperativoCreate).toHaveBeenCalledWith(
  ...
  describe('getEventoById', () => {
    it('throws NotFoundError when evento does not exist', async () => {
      ...
      await expect(eventoService.getEventoById(EVENTO_ID, ORG_ID, USER_ID)).rejects.toThrow(
        NotFoundError
      );
    });
  ...
  describe('listEventos', () => {
    it('filters by prestadorId when user is prestador', async () => {
      ...
      expect(mockEventoOperativoFindAndCountAll).toHaveBeenCalledWith(
        expect.objectContaining({
