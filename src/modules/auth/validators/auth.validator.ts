...
export const RegisterSchema = registry.register(
  ...
  z
    ...
    .refine(
      (data) => {
        if (data.invitationId == null) return true;
      ...
      {
        message:
          'Con invitationId debes enviar o bien token (enlace) o bien invitationProof (tras verificar email con código), pero no ambos',
        path: ['invitationId'],
      }
    )
    .refine(
      (data) => data.invitationId == null || data.token != null || data.invitationProof != null,
      {
