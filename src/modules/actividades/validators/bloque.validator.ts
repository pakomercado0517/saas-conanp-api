...
export const CreateBloqueSchema = z
  ...
  .refine(
    ...
    {
      message:
        'Si es plantilla (isTemplate=true), la fecha debe ser null. Si no es plantilla (isTemplate=false), la fecha es requerida',
      path: ['date'],
    }
  )
  .refine(
...
export const UpdateBloqueSchema = z
  ...
  .refine(
    ...
    {
      message:
        'Si es plantilla (isTemplate=true), la fecha debe ser null. Si no es plantilla (isTemplate=false), la fecha no puede ser null',
      path: ['date'],
    }
  )
...
const SORT_FIELDS = ['date', 'startTime', 'endTime', 'capacity', 'createdAt', 'updatedAt'] as const;
