...
export const createActividad = async (req: Request, res: Response): Promise<Response> => {
  ...
  const data = req.body as CreateActividadDTO;
 ...
 * }
 */
export const getActividadById = async (req: Request, res: Response): Promise<Response> => {
  if (!req.user) {
    return res.status(401).json({
      success: false,
      error: 'No autorizado',
 ...
 * }
 */
export const listActividades = async (req: Request, res: Response): Promise<Response> => {
  if (!req.user) {
    return res.status(401).json({
      success: false,
      error: 'No autorizado',
  ...
  const filters =
    (req.validatedQuery as ListActividadesDTO | undefined) ??
    (req.query as unknown as ListActividadesDTO);
 ...
 * }
 */
export const updateActividad = async (req: Request, res: Response): Promise<Response> => {
  if (!req.user) {
    return res.status(401).json({
      success: false,
      error: 'No autorizado',
  ...
  const actividadId = req.params['actividadId'] as string;
  const userId = req.user.userId;
  const data = req.body as UpdateActividadDTO;

  const actividad = await actividadService.updateActividad(
    actividadId,
    organizationId,
    data,
    userId
 ...
 */
export const deleteActividad = async (req: Request, res: Response): Promise<Response> => {
  if (!req.user) {
    return res.status(401).json({
      success: false,
      error: 'No autorizado',
