 ...
 */
const validateActividadHasBloquesType = async (
  actividadId: UUID,
  organizationId: UUID
): Promise<void> => {
  ...
  if (actividad.agendaType !== 'BLOQUES') {
    throw new ValidationError(
      'Solo las actividades con tipo de agenda BLOQUES pueden tener bloques',
      undefined,
      { actividadId, agendaType: actividad.agendaType }
    );
  }
};
 ...
 */
const validateNoTimeOverlap = async (
  actividadId: UUID,
  date: string | DateTime | null,
  startTime: string | DateTime,
  endTime: string | DateTime,
  organizationId: UUID,
  excludeBloqueId?: UUID
): Promise<void> => {
  ...
  let startTimeDt: DateTime | null = null;
  let endTimeDt: DateTime | null = null;
  ...
  for (const existingBloque of existingBloques) {
    const existingStartTime = parseTimeOnly(
      existingBloque.startTime,
      parseDateOnly(existingBloque.date || '') || undefined
    );
    const existingEndTime = parseTimeOnly(
      existingBloque.endTime,
      parseDateOnly(existingBloque.date || '') || undefined
    );
    ...
    if (doTimeRangesOverlap(startTimeDt, endTimeDt, existingStartTime, existingEndTime)) {
      throw new ValidationError(
        `El bloque se solapa con otro bloque existente (${existingBloque.startTime} - ${existingBloque.endTime})`,
        undefined,
        {
 ...
 */
export const getBloquesTemplates = async (
  actividadId: UUID,
  organizationId: UUID
): Promise<Bloque[]> => {
 ...
 */
export const createBloqueFromTemplate = async (
  data: CreateBloqueFromTemplateDTO,
  organizationId: UUID,
  userId: UUID
): Promise<Bloque> => {
  // Validar que el usuario sea admin
  ...
  if (template.Actividad && template.Actividad.agendaType !== 'BLOQUES') {
    throw new ValidationError(
      'La plantilla pertenece a una actividad que no tiene tipo de agenda BLOQUES',
      undefined,
      {
 ...
 * @throws {NotFoundError} Si la actividad no existe
 */
export const listBloquesByActividad = async (
  actividadId: UUID,
  organizationId: UUID,
  filters: Omit<ListBloquesDTO, 'actividadId' | 'organizationId'>,
  userId: UUID
): Promise<{ data: Bloque[]; pagination: PaginationMeta }> => {
  // Validar acceso a la organización
  await assertCanAccessOrganization(userId, organizationId);
 ...
 * @throws {ForbiddenError} Si no tiene acceso a la organización
 */
export const listBloques = async (
  organizationId: UUID,
  filters: ListBloquesDTO,
  userId: UUID
): Promise<{ data: Bloque[]; pagination: PaginationMeta }> => {
  // Validar acceso a la organización
  await assertCanAccessOrganization(userId, organizationId);
...
export const updateBloque = async (
  ...
  let newDate: string | null = bloque.date;
 ...
 * @throws {NotFoundError} Si el bloque no existe o no pertenece a la organización
 */
export const deleteBloque = async (
  bloqueId: UUID,
  organizationId: UUID,
  userId: UUID
): Promise<void> => {
