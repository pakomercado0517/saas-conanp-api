...
const ACTIVIDAD_ID = 'cccccccc-cccc-cccc-cccc-cccccccccccc' as UUID;
const BLOQUE_ID = 'dddddddd-dddd-dddd-dddd-dddddddddddd' as UUID;
const TEMPLATE_ID = 'eeeeeeee-eeee-eeee-eeee-eeeeeeeeeeee' as UUID;
...
vi.mock('@/modules/actividades/models/bloque.model.js', () => ({
  Bloque: {
    ...
    findAll: (...args: unknown[]): unknown => mockBloqueFindAll(...args),
    count: (...args: unknown[]): unknown => mockBloqueCount(...args),
    create: (...args: unknown[]): unknown => mockBloqueCreate(...args),
    findAndCountAll: (...args: unknown[]): unknown => mockBloqueFindAndCountAll(...args),
  },
}));
...
describe('bloque.service', () => {
  ...
  describe('listBloquesByActividad (materialización bajo demanda)', () => {
    const baseFilters = {
      ...
      date: undefined as DateTime | undefined,
    };
    ...
    it('no materializa cuando la actividad es HORARIO_LIBRE', async () => {
      ...
      await bloqueService.listBloquesByActividad(
        ACTIVIDAD_ID,
        ORG_ID,
        { ...baseFilters, date: DateTime.fromISO('2026-01-15') },
        USER_ID
      );
    ...
    it('lanza NotFoundError cuando la actividad no existe', async () => {
      ...
      await expect(
        bloqueService.listBloquesByActividad(
          ACTIVIDAD_ID,
          ORG_ID,
          { ...baseFilters, date: DateTime.fromISO('2026-01-15') },
          USER_ID
  ...
  describe('createBloqueFromTemplate', () => {
    it('crea un bloque desde plantilla y lo devuelve (usa materializeBloqueFromTemplate)', async () => {
      ...
      const result = await bloqueService.createBloqueFromTemplate(
        { templateId: TEMPLATE_ID, date: DateTime.fromISO('2026-01-20') },
        ORG_ID,
        USER_ID
      );
    ...
    it('lanza NotFoundError cuando la plantilla no existe', async () => {
      ...
      await expect(
        bloqueService.createBloqueFromTemplate(
          { templateId: TEMPLATE_ID, date: DateTime.fromISO('2026-01-20') },
          ORG_ID,
          USER_ID
        )
