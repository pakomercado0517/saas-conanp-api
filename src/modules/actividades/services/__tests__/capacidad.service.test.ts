...
const CAPACIDAD_ID = 'eeeeeeee-eeee-eeee-eeee-eeeeeeeeeeee' as UUID;
...
describe('capacidad.service', () => {
  ...
  describe('updateCapacidad', () => {
    it('throws NotFoundError when capacidad does not exist', async () => {
      ...
      await expect(
        capacidadService.updateCapacidad(CAPACIDAD_ID, ORG_ID, { limit: 20 }, USER_ID)
    ...
    it('updates capacidad and returns it on success', async () => {
      ...
      const result = await capacidadService.updateCapacidad(
        CAPACIDAD_ID,
        ORG_ID,
        { limit: 20 },
        USER_ID
  ...
  describe('verificarDisponibilidadPorBloque', () => {
    it('throws NotFoundError when actividad does not exist', async () => {
      ...
      await expect(
        capacidadService.verificarDisponibilidadPorBloque(
          ACTIVIDAD_ID,
          BLOQUE_ID,
          '2025-02-01',
          1,
    ...
    it('returns disponible true when capacity available (uses bloque.capacity when no Capacidad)', async () => {
      ...
      const result = await capacidadService.verificarDisponibilidadPorBloque(
        ACTIVIDAD_ID,
        BLOQUE_ID,
        '2025-02-01',
        5,
  ...
  describe('verificarDisponibilidadPorDia', () => {
    it('throws NotFoundError when actividad does not exist', async () => {
      ...
      await expect(
        capacidadService.verificarDisponibilidadPorDia(ACTIVIDAD_ID, '2025-02-01', 1, ORG_ID)
      ).rejects.toThrow(NotFoundError);
    });
    ...
    it('returns disponible true when capacity available', async () => {
      ...
      const result = await capacidadService.verificarDisponibilidadPorDia(
        ACTIVIDAD_ID,
        '2025-02-01',
        10,
        ORG_ID
