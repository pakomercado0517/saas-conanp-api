 ...
 */
const calcularCapacidadUsada = async (
  actividadId: UUID,
  date: string,
  organizationId: UUID,
  bloqueId?: UUID | null
 ...
 */
export const createCapacidad = async (
  data: CreateCapacidadDTO,
  organizationId: UUID,
  userId: UUID
): Promise<Capacidad> => {
 ...
 */
export const updateCapacidad = async (
  capacidadId: UUID,
  organizationId: UUID,
  data: UpdateCapacidadDTO,
  userId: UUID
): Promise<Capacidad> => {
  // Validar que el usuario sea admin
  await assertIsAdmin(userId, organizationId);
 ...
 */
export const verificarDisponibilidadPorBloque = async (
  actividadId: UUID,
  bloqueId: UUID,
  ...
  if (actividad.agendaType !== 'BLOQUES') {
    throw new ValidationError(
      'Esta función solo aplica para actividades con tipo de agenda BLOQUES',
      undefined,
 ...
 */
export const verificarDisponibilidadPorDia = async (
  actividadId: UUID,
  date: DateTime | string,
  cantidad: number,
  organizationId: UUID
  ...
  if (actividad.agendaType !== 'HORARIO_LIBRE') {
    throw new ValidationError(
      'Esta función solo aplica para actividades con tipo de agenda HORARIO_LIBRE',
      undefined,
      { actividadId, agendaType: actividad.agendaType }
    );
  }
  ...
  if (!capacidad) {
    throw new NotFoundError('Capacidad', {
      ...
      organizationId,
      message:
        'No existe una capacidad definida para esta actividad y fecha. Debe crearse primero.',
    });
  }
 ...
 */
export const actualizarCapacidadUsada = async (
  actividadId: UUID,
  date: string,
  organizationId: UUID,
  bloqueId?: UUID | null
