...
export const validateBrevoConfig = () => {
    try {
        getConfig();
        logger.info({ senderEmail: config.senderEmail, senderName: config.senderName }, 'Configuración de Brevo validada');
        return true;
    }
    catch (error) {
        logger.error({ error: error instanceof Error ? error.message : String(error) }, 'Error en configuración de Brevo');
        return false;
    }
};
