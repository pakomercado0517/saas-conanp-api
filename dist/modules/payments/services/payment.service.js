 ...
 * @returns Pago creado
 */
const savePayment = async (data, transaction) => {
 ...
 */
export const createPaymentIntent = async (data, organizationId, userId) => {
 ...
 */
export const confirmPayment = async (data, organizationId, userId) => {
    // 1. Validar acceso a la organización
    await assertCanAccessOrganization(userId, organizationId);
    ...
    try {
        ...
        if (confirmedPaymentIntent.latest_charge) {
            const charge = typeof confirmedPaymentIntent.latest_charge === 'string'
 ...
 * @throws {NotFoundError} Si el pago no existe o no pertenece a la organización
 */
export const getPaymentById = async (paymentId, organizationId, userId) => {
 ...
 * @throws {ForbiddenError} Si no tiene acceso a la organización
 */
export const listPayments = async (organizationId, filters, userId) => {
    // Validar acceso a la organización
    await assertCanAccessOrganization(userId, organizationId);
    ...
    if (filters.dateFrom || filters.dateTo) {
        const dateFilter = {};
        if (filters.dateFrom) {
            const dateFromStr = typeof filters.dateFrom === 'string' ? filters.dateFrom : filters.dateFrom.toISODate();
            dateFilter[Op.gte] = dateFromStr;
        }
        if (filters.dateTo) {
            const dateToStr = typeof filters.dateTo === 'string' ? filters.dateTo : filters.dateTo.toISODate();
            dateFilter[Op.lte] = dateToStr;
        }
    ...
    if (filters.amountMin != null || filters.amountMax != null) {
        ...
        if (filters.amountMin != null) {
            amountFilter[Op.gte] = filters.amountMin;
        }
        if (filters.amountMax != null) {
            amountFilter[Op.lte] = filters.amountMax;
        }
 ...
 */
export const processRefund = async (data, organizationId, userId) => {
    // 1. Validar acceso a la organización
    await assertCanAccessOrganization(userId, organizationId);
    // 2. Buscar pago por ID con filtro multi-tenant
    const payment = await Payment.findOne({
    ...
    if (payment.status !== 'succeeded') {
        throw new ValidationError(`No se puede reembolsar un pago en estado '${payment.status}'. Solo se pueden reembolsar pagos en estado 'succeeded'`, 'status');
    }
    ...
    if (refundAmount > availableAmount) {
        throw new ValidationError(`El monto a reembolsar (${refundAmount} centavos) excede el monto disponible (${availableAmount} centavos)`, 'amount');
    }
    if (refundAmount <= 0) {
 ...
 */
export const ensureEventIdempotency = async (eventId, eventType) => {
    try {
 ...
 */
export const handleChargeRefundedFromWebhook = async (charge) => {
    const paymentIntent = typeof charge['payment_intent'] === 'string'
        ? charge['payment_intent']
        : charge['payment_intent']?.id;
    const amountRefunded = typeof charge['amount_refunded'] === 'number' ? charge['amount_refunded'] : 0;
    if (!paymentIntent || typeof paymentIntent !== 'string') {
        logger.warn({ chargeId: charge['id'] ?? 'unknown' }, 'Webhook charge.refunded: payment_intent ausente o inválido');
        return null;
    }
    const payment = await Payment.findOne({
 ...
 */
export const updatePaymentStatusFromWebhook = async (stripePaymentIntentId, eventType, eventData) => {
    ...
    switch (eventType) {
        ...
        case 'payment_intent.payment_failed':
            newStatus = 'failed';
            failureReason =
                eventData['last_payment_error']?.message || 'El pago falló';
            break;
