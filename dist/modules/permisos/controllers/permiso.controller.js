...
export const listPermisos = async (req, res) => {
    ...
    if (!filters.prestadorId) {
        throw new ValidationError('El ID de prestador (prestadorId) es requerido en los query params para listar permisos', undefined, {
            prestadorId: 'requerido',
        });
    }
    const result = await permisoService.listPermisosByPrestador(filters.prestadorId, organizationId, filters, userId);
