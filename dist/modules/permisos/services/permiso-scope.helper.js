 ...
 */
export const normalizeActivityName = (name) => name.trim().replace(/\s+/g, ' ').toLocaleLowerCase('es-MX');
/**
 ...
 */
export const findMatchingActividadesByDependencia = async (dependenciaId, referenceActividadId) => {
    ...
    if (!refArea || refArea.dependenciaId !== dependenciaId) {
        throw new ValidationError('La actividad de referencia no pertenece a una área de esta dependencia', undefined, {
            actividadId: referenceActividadId,
            dependenciaId,
        });
    }
    ...
    if (missing.length > 0) {
        throw new ValidationError('No existe una actividad con el mismo nombre en todas las áreas de la dependencia. Ajusta los nombres o crea la actividad faltante.', undefined, {
...
//# sourceMappingURL=permiso-scope.helper.js.map