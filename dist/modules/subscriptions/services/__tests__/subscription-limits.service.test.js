...
describe('subscription-limits.service', () => {
    ...
    describe('getOrganizationUsage', () => {
        ...
        it('returns usage with period bounds for eventos', async () => {
            ...
            expect(mockEventoOperativoCount).toHaveBeenCalledWith(expect.objectContaining({
    ...
    describe('checkUsersLimit', () => {
        it('throws NotFoundError when no active subscription', async () => {
            mockSubscriptionFindOne.mockResolvedValueOnce(null);
            await expect(subscriptionLimitsService.checkUsersLimit(ORG_ID)).rejects.toThrow(NotFoundError);
        });
        it('does not throw when plan has no maxUsers limit', async () => {
            mockSubscriptionFindOne.mockResolvedValueOnce(activeSubscriptionWithPlan({
                id: PLAN_ID,
                name: 'básico',
                maxUsers: null,
                maxEventos: null,
        ...
        it('throws ValidationError when usage >= maxUsers', async () => {
            ...
            await expect(subscriptionLimitsService.checkUsersLimit(ORG_ID)).rejects.toThrow(ValidationError);
        });
        ...
        it('accepts currentCount override', async () => {
            ...
            await expect(subscriptionLimitsService.checkUsersLimit(ORG_ID, 10)).rejects.toThrow(ValidationError);
        });
    });
    describe('checkEventosLimit', () => {
        it('throws NotFoundError when no active subscription', async () => {
            mockSubscriptionFindOne.mockResolvedValueOnce(null);
            await expect(subscriptionLimitsService.checkEventosLimit(ORG_ID)).rejects.toThrow(NotFoundError);
        });
        ...
        it('throws ValidationError when eventos count >= maxEventos', async () => {
            ...
            mockActividadCount.mockResolvedValue(0);
            await expect(subscriptionLimitsService.checkEventosLimit(ORG_ID)).rejects.toThrow(ValidationError);
        });
        it('does not throw when currentCount passed and under limit', async () => {
            ...
            await expect(subscriptionLimitsService.checkEventosLimit(ORG_ID, 50)).resolves.toBeUndefined();
        });
    });
    describe('checkActividadesLimit', () => {
        it('throws NotFoundError when no active subscription', async () => {
            mockSubscriptionFindOne.mockResolvedValueOnce(null);
            await expect(subscriptionLimitsService.checkActividadesLimit(ORG_ID)).rejects.toThrow(NotFoundError);
        });
        it('does not throw when plan has no maxActividades limit', async () => {
            ...
            await expect(subscriptionLimitsService.checkActividadesLimit(ORG_ID)).resolves.toBeUndefined();
        });
        it('throws ValidationError when actividades count >= maxActividades', async () => {
            ...
            await expect(subscriptionLimitsService.checkActividadesLimit(ORG_ID)).rejects.toThrow(ValidationError);
        });
    });
    describe('checkOrganizationsLimit', () => {
        it('does not throw when plan has no maxOrganizations limit', async () => {
            ...
            await expect(subscriptionLimitsService.checkOrganizationsLimit(PLAN_ID)).resolves.toBeUndefined();
        });
        it('throws ValidationError when count >= maxOrganizations', async () => {
            ...
            await expect(subscriptionLimitsService.checkOrganizationsLimit(PLAN_ID)).rejects.toThrow(ValidationError);
        });
...
//# sourceMappingURL=subscription-limits.service.test.js.map