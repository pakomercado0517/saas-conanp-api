...
export const createBloque = async (req, res) => {
    ...
    const actividadId = req.params['actividadId'] || data.actividadId;
 ...
 * }
 */
export const createBloqueFromTemplate = async (req, res) => {
    if (!req.user) {
        return res.status(401).json({
            success: false,
            error: 'No autorizado',
 ...
 * }
 */
export const getBloqueById = async (req, res) => {
    if (!req.user) {
        return res.status(401).json({
            success: false,
            error: 'No autorizado',
 ...
 * }
 */
export const listBloques = async (req, res) => {
    if (!req.user) {
        return res.status(401).json({
            success: false,
            error: 'No autorizado',
 ...
 * }
 */
export const listBloquesByActividad = async (req, res) => {
    if (!req.user) {
        return res.status(401).json({
            success: false,
            error: 'No autorizado',
    ...
    const filters = req.validatedQuery ??
        req.query;
    const result = await bloqueService.listBloquesByActividad(actividadId, organizationId, filters, userId);
    return sendPaginated(res, result.data, result.pagination, 'Bloques obtenidos exitosamente');
};
/**
