 ...
 * }
 */
export const createOrUpdateCapacidad = async (req, res) => {
    if (!req.user) {
    ...
    try {
        ...
        if (error instanceof ConflictError) {
            ...
            const updatedCapacidad = await capacidadService.updateCapacidad(existingCapacidad.id, organizationId, {
                limit: data.limit,
            }, userId);
...
export const verificarDisponibilidad = async (req, res) => {
    ...
    const bloqueId = req.query['bloqueId'];
    const cantidad = req.query['cantidad'] ? Number.parseInt(req.query['cantidad'], 10) : 1;
    ...
    if (actividad.agendaType === 'BLOQUES') {
        ...
        resultado = await capacidadService.verificarDisponibilidadPorBloque(actividadId, bloqueId, date, cantidad, organizationId);
    }
    else if (actividad.agendaType === 'HORARIO_LIBRE') {
        resultado = await capacidadService.verificarDisponibilidadPorDia(actividadId, date, cantidad, organizationId);
    }
    else {
        return res.status(400).json({
            success: false,
