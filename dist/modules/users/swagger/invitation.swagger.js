 ...
 */
const ValidateInvitationResponseSchema = registry.register('ValidateInvitationResponse', z.object({
 ...
 */
const StartVerifyEmailResponseSchema = registry.register('StartVerifyEmailResponse', z.object({
    success: z.literal(true),
 ...
 */
const ConfirmVerifyEmailResponseSchema = registry.register('ConfirmVerifyEmailResponse', z.object({
    success: z.literal(true),
    data: z.object({
...
registry.registerPath({
    ...
    description: 'Valida el token de una invitación (enlace). Devuelve datos de la organización y rol para mostrar el formulario de registro. Usar con invitationId + token en POST /auth/register (flujo enlace).',
    request: {
        body: {
            content: {
                'application/json': {
...
registry.registerPath({
    ...
    description: 'Envía un código OTP al email de la invitación. Usar cuando el usuario ingresa el código de invitación manualmente; tras recibir el OTP, llamar a POST verify-email/confirm y luego registrar con invitationProof.',
    request: {
        body: {
            content: {
                'application/json': {
...
registry.registerPath({
    ...
    description: 'Valida el código OTP recibido por correo y devuelve un invitationProof de un solo uso. Enviar invitationId, email e invitationProof en POST /auth/register para completar el registro (email quedará verificado).',
    request: {
        body: {
            content: {
                'application/json': {
...
//# sourceMappingURL=invitation.swagger.js.map