...
const assertProductoBelongsToDependencia = async (dependenciaId, productoAccesoId) => {
...
const getOrCreateStock = async (dependenciaId, productoAccesoId) => {
...
const assertPrestadorBelongsToDependencia = async (dependenciaId, prestadorId) => {
...
const assertEventoBelongsToArea = async (areaId, eventoId) => {
 ...
 */
export const registrarEntrada = async (areaId, productoAccesoId, data, userId) => {
 ...
 */
export const registrarSalida = async (areaId, productoAccesoId, data, userId) => {
    await assertCanAccessOrganization(userId, areaId);
    const dependenciaId = await getDependenciaIdFromAreaId(areaId);
    await assertProductoBelongsToDependencia(dependenciaId, productoAccesoId);
    if (data.prestadorId) {
 ...
 */
export const getStockDisponible = async (areaId, productoAccesoId, userId) => {
    await assertCanAccessOrganization(userId, areaId);
    const dependenciaId = await getDependenciaIdFromAreaId(areaId);
    await assertProductoBelongsToDependencia(dependenciaId, productoAccesoId);
    const stock = await StockAcceso.findOne({
 ...
 */
export const listMovimientos = async (areaId, filters, userId) => {
    await assertCanAccessOrganization(userId, areaId);
    const dependenciaId = await getDependenciaIdFromAreaId(areaId);
    const where = {
        dependenciaId,
