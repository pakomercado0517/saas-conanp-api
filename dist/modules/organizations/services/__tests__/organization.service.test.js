...
vi.mock('@/modules/organizations/models/organization.model.js', () => ({
    Organization: {
        create: (...args) => mockOrganizationCreate(...args),
        findByPk: (...args) => mockOrganizationFindByPk(...args),
        findAndCountAll: (...args) => mockOrganizationFindAndCountAll(...args),
    },
}));
...
describe('organization.service', () => {
    ...
    describe('assertCanAccessOrganization', () => {
        it('throws ForbiddenError when no active membership', async () => {
            ...
            await expect(organizationService.assertCanAccessOrganization(USER_ID, ORG_ID)).rejects.toThrow(ForbiddenError);
        });
        it('does not throw when membership exists', async () => {
            ...
            await expect(organizationService.assertCanAccessOrganization(USER_ID, ORG_ID)).resolves.toBeUndefined();
        });
    });
    ...
    describe('createOrganization', () => {
        it('creates organization without access check', async () => {
            ...
            mockOrganizationCreate.mockResolvedValueOnce(createdOrg);
    ...
    describe('getOrganizationById', () => {
        it('throws when assertCanAccessOrganization rejects', async () => {
            mockMembershipFindOne.mockResolvedValueOnce(null);
            await expect(organizationService.getOrganizationById(ORG_ID, USER_ID)).rejects.toThrow(ForbiddenError);
        });
        it('throws when assertActiveSubscription rejects', async () => {
            mockMembershipFindOne.mockResolvedValueOnce({ userId: USER_ID, organizationId: ORG_ID });
            mockSubscriptionFindOne.mockResolvedValueOnce(null);
            await expect(organizationService.getOrganizationById(ORG_ID, USER_ID)).rejects.toThrow(ForbiddenError);
        });
        it('throws NotFoundError when organization does not exist', async () => {
            ...
            await expect(organizationService.getOrganizationById(ORG_ID, USER_ID)).rejects.toThrow(NotFoundError);
        });
        it('returns organization when found', async () => {
            ...
            mockOrganizationFindByPk.mockResolvedValueOnce(orgRecord);
    ...
    describe('listOrganizations', () => {
        it('returns empty list when user has no memberships', async () => {
            ...
            const result = await organizationService.listOrganizations({ page: 1, limit: 10, sortOrder: 'desc', name: undefined }, USER_ID);
            expect(result.data).toEqual([]);
            expect(result.pagination.total).toBe(0);
            expect(mockOrganizationFindAndCountAll).not.toHaveBeenCalled();
        });
        it('returns only organizations where user has membership', async () => {
            mockMembershipFindAll.mockResolvedValueOnce([{ organizationId: ORG_ID }]);
            mockOrganizationFindAndCountAll.mockResolvedValueOnce({
                rows: [
                    {
                        id: ORG_ID,
                        name: 'Test Org',
