 ...
 */
const validateFile = (_file, contentType, fileSize) => {
    // Validar tamaño
    if (fileSize > MAX_FILE_SIZE) {
        throw new ValidationError(`El archivo excede el tamaño máximo permitido de ${MAX_FILE_SIZE / (1024 * 1024)}MB`, 'file', { fileSize, maxSize: MAX_FILE_SIZE });
    }
    // Validar tipo MIME
    if (!ALLOWED_MIME_TYPES.includes(contentType)) {
        throw new ValidationError(`Tipo de archivo no permitido. Tipos permitidos: ${ALLOWED_MIME_TYPES.join(', ')}`, 'contentType', { contentType, allowedTypes: ALLOWED_MIME_TYPES });
    }
};
/**
 ...
 */
export const createEvidencia = async (data, organizationId, userId, file, contentType) => {
    ...
    if (file && contentType) {
        ...
        if (!extension) {
            throw new ValidationError(`No se pudo determinar la extensión para el tipo MIME: ${contentType}`, 'contentType', { contentType });
        }
        // Generar key único
 ...
 */
export const getEvidenciaById = async (evidenciaId, organizationId, userId) => {
    // Validar acceso a la organización
    await assertCanAccessOrganization(userId, organizationId);
 ...
 */
export const listEvidencias = async (organizationId, filters, userId) => {
    // Validar acceso a la organización
    await assertCanAccessOrganization(userId, organizationId);
 ...
 * @throws {ValidationError} Si las validaciones de archivo fallan
 */
export const updateEvidencia = async (evidenciaId, organizationId, data, userId, file, contentType) => {
    // Validar acceso a la organización
    await assertCanAccessOrganization(userId, organizationId);
 ...
 * @throws {NotFoundError} Si la evidencia no existe o el evento no pertenece a la organización
 */
export const deleteEvidencia = async (evidenciaId, organizationId, userId) => {
    // Validar acceso a la organización
    await assertCanAccessOrganization(userId, organizationId);
    // Obtener evidencia y validar que existe y pertenece a la organización
    const evidencia = await getEvidenciaById(evidenciaId, organizationId, userId);
 ...
 */
export const getEvidenciaSignedUrl = async (evidenciaId, organizationId, userId, expiresIn = 3600) => {
...
//# sourceMappingURL=evidencia.service.js.map