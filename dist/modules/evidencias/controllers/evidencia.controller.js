 ...
 * }
 */
export const listEvidencias = async (req, res) => {
    if (!req.user) {
        return res.status(401).json({
            success: false,
            error: 'No autorizado',
...
export const updateEvidencia = async (req, res) => {
    ...
    if (req.body['fileUrl'] !== undefined) {
        ...
        const fileUrlValue = req.body['fileUrl'];
    ...
    const evidencia = await evidenciaService.updateEvidencia(evidenciaId, organizationId, data, userId, fileBuffer, contentType);
...
//# sourceMappingURL=evidencia.controller.js.map